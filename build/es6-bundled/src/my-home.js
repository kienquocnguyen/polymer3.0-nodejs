import{PolymerElement,html}from"./my-app.js";class MyHome extends PolymerElement{constructor(){super();this.page=0}static get properties(){return{queryParams:{type:Number,value:"1"},currentPage:{type:Number,value:2},routeData:Object,subroute:Object}}static get template(){return html`
    ...
    `}static get observers(){return["_currentPageChange(current)"]}gotoSingle(e){const permalinks=e.model.item.permalinks;location.href=`/single-post/?post=${permalinks}`}_currentPageChange(c){let location=this.$.location,path=location.path;this.page=c;var params=this.queryParams;if(1!=this.page){params=Math.pow(2,this.page);location.path=`/home/${this.page}`;fetch(`https://api.mypolymerblog.com/posts/${params}`).then(res=>res.json()).then(myposts=>this.myposts=myposts)}else{params=0;location.path=`/home/${this.page}`;fetch(`https://api.mypolymerblog.com/posts/${params}`).then(res=>res.json()).then(myposts=>this.myposts=myposts);console.log(this.page)}}connectedCallback(e){var params=this.queryParams;super.connectedCallback();fetch(`https://api.mypolymerblog.com/posts/${params}`).then(res=>res.json()).then(myposts=>this.myposts=myposts);console.log(params);fetch(`https://api.mypolymerblog.com/postscount`).then(res=>res.json()).then(postcount=>this.postcount=postcount);fetch(`https://api.mypolymerblog.com/latest/posts`).then(res=>res.json()).then(latestposts=>this.latestposts=latestposts)}}window.customElements.define("my-home",MyHome);